...
export function parentOfTypeWithCb<T>(
    node: SyntaxNode,
    cb: (parent: SyntaxNode, branch: SyntaxNode) => T,
...
export function closestNamedSibling(
    node: SyntaxNode,
    direction: "prev" | "next",
    cb: (sibling: SyntaxNode) => boolean,
): SyntaxNode | null {
    ...
    while (curSibling) {
        ...
        curSibling =
            direction == "prev" ? curSibling.previousNamedSibling : curSibling.nextNamedSibling
    }
    return null
}
